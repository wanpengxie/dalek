...
const permTimeoutDefault = 12 * time.Second

func cmdPerm(args []string) {
	if len(args) == 0 {
		printPermUsage()
		os.Exit(2)
	}
	...
	default:
		exitUsageError(globalOutput,
			fmt.Sprintf("未知 perm 子命令: %s", sub),
			"perm 仅支持 share|add|ls",
			"运行 feishu perm --help 查看可用命令",
		)
	}
}

func printPermUsage() {
	printGroupUsage("飞书权限管理", "feishu perm <command> [flags]", []string{
		"share    更新公开分享设置并返回可访问链接",
		"add      添加协作者",
		"ls       列出协作者权限",
	})
	fmt.Fprintln(os.Stderr, "Use \"feishu perm <command> --help\" for more information.")
}

func cmdPermShare(args []string) {
	...
	fs.Usage = func() {
		printSubcommandUsage(
			fs,
			"更新公开分享设置",
			"feishu perm share --url <飞书链接> [--link-share tenant_editable] [--external-access] [--timeout 12s] [--output text|json]",
			"feishu perm share --url https://feishu.cn/docx/xxxxxxxx",
			"feishu perm share --url https://feishu.cn/docx/xxxxxxxx --link-share anyone_editable --external-access -o json",
		)
	}
	...
	timeout := fs.Duration("timeout", permTimeoutDefault, "请求超时（例如 12s）")
	output := addOutputFlag(fs, "输出格式: text|json（默认 text）")
	parseFlagSetOrExit(fs, args, globalOutput, "feishu perm share 参数解析失败", "运行 feishu perm share --help 查看参数")
	out := parseOutputOrExit(*output, true)
	resolvedToken, resolvedType := resolveTokenAndType(out, *urlFlag, *token, *tokenType, "例如: feishu perm share --url https://feishu.cn/docx/xxxxxxxx")
	if *timeout <= 0 {
		exitUsageError(out, "非法参数 --timeout", "--timeout 必须大于 0", "例如: feishu perm share --timeout 12s")
	}
...
func cmdPermAdd(args []string) {
	...
	fs.Usage = func() {
		printSubcommandUsage(
			fs,
			"添加协作者",
			"feishu perm add --url <飞书链接> --member-type <type> --member-id <id> [--perm edit] [--perm-type container] [--collab-type user] [--notify] [--timeout 12s] [--output text|json]",
			"feishu perm add --url https://feishu.cn/docx/xxxxxxxx --member-type openid --member-id ou_xxxxx --perm edit",
			"feishu perm add --token doxcxxxxxxxx --member-type email --member-id user@example.com --perm view -o json",
		)
	}
	home := fs.String("home", globalHome, "dalek Home 目录（默认 ~/.dalek）")
	urlFlag := fs.String("url", "", "飞书文档链接（与 --token 二选一，自动识别类型）")
	...
	timeout := fs.Duration("timeout", permTimeoutDefault, "请求超时（例如 12s）")
	output := addOutputFlag(fs, "输出格式: text|json（默认 text）")
	parseFlagSetOrExit(fs, args, globalOutput, "feishu perm add 参数解析失败", "运行 feishu perm add --help 查看参数")
	out := parseOutputOrExit(*output, true)
	resolvedToken, resolvedType := resolveTokenAndType(out, *urlFlag, *token, *tokenType, "例如: feishu perm add --url https://feishu.cn/docx/xxxxxxxx --member-type openid --member-id ou_xxxxx")
	if strings.TrimSpace(*memberType) == "" {
		exitUsageError(out, "缺少必填参数 --member-type", "--member-type 不能为空", "例如: feishu perm add --member-type openid")
	}
	if strings.TrimSpace(*memberID) == "" {
		exitUsageError(out, "缺少必填参数 --member-id", "--member-id 不能为空", "例如: feishu perm add --member-id ou_xxxxx")
	}
	if *timeout <= 0 {
		exitUsageError(out, "非法参数 --timeout", "--timeout 必须大于 0", "例如: feishu perm add --timeout 12s")
	}
...
func cmdPermList(args []string) {
	...
	fs.Usage = func() {
		printSubcommandUsage(
			fs,
			"列出协作者权限",
			"feishu perm ls --url <飞书链接> [--fields name,type,avatar,external_label] [--perm-type container] [--timeout 12s] [--output text|json]",
			"feishu perm ls --url https://feishu.cn/docx/xxxxxxxx",
			"feishu perm ls --token doxcxxxxxxxx --perm-type single_page -o json",
		)
	}
	home := fs.String("home", globalHome, "dalek Home 目录（默认 ~/.dalek）")
	urlFlag := fs.String("url", "", "飞书文档链接（与 --token 二选一，自动识别类型）")
	...
	timeout := fs.Duration("timeout", permTimeoutDefault, "请求超时（例如 12s）")
	output := addOutputFlag(fs, "输出格式: text|json（默认 text）")
	parseFlagSetOrExit(fs, args, globalOutput, "feishu perm ls 参数解析失败", "运行 feishu perm ls --help 查看参数")
	out := parseOutputOrExit(*output, true)
	resolvedToken, resolvedType := resolveTokenAndType(out, *urlFlag, *token, *tokenType, "例如: feishu perm ls --url https://feishu.cn/docx/xxxxxxxx")
	if *timeout <= 0 {
		exitUsageError(out, "非法参数 --timeout", "--timeout 必须大于 0", "例如: feishu perm ls --timeout 12s")
	}
