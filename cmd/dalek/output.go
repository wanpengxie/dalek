...
func normalizeThreePartError(what, cause, fix string) (string, string, string) {
	...
	if fix == "" {
		fix = "运行 dalek --help 查看可用命令"
	}
	return what, cause, fix
}

func printThreePartError(what, cause, fix string) {
	what, cause, fix = normalizeThreePartError(what, cause, fix)
	fmt.Fprintf(os.Stderr, "Error: %s\nCause: %s\nFix: %s\n", what, cause, fix)
}

func printFixFirstError(what, fix, cause string) {
	what, cause, fix = normalizeThreePartError(what, cause, fix)
	fmt.Fprintf(os.Stderr, "Error: %s\nFix: %s\nCause: %s\n", what, fix, cause)
}

func writeErrorJSONIfNeeded(out cliOutputFormat, exitCode int, what, cause, fix string) {
	if out == outputJSON {
		printJSONOrExit(cliErrorJSON{
			Schema:   "dalek.error.v1",
			Error:    strings.TrimSpace(what),
			Cause:    strings.TrimSpace(cause),
			Fix:      strings.TrimSpace(fix),
			ExitCode: exitCode,
		})
...
func exitWithError(out cliOutputFormat, exitCode int, what, cause, fix string) {
	printThreePartError(what, cause, fix)
	writeErrorJSONIfNeeded(out, exitCode, what, cause, fix)
	os.Exit(exitCode)
}

func exitFixFirstError(out cliOutputFormat, exitCode int, what, fix, cause string) {
	printFixFirstError(what, fix, cause)
	writeErrorJSONIfNeeded(out, exitCode, what, cause, fix)
	os.Exit(exitCode)
}
