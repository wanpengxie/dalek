...
> 本文件用于记录“当前开发任务、执行计划、进度与自检证据”。  
> 约束：本次执行必须严格遵循 `docs/INTERNAL_ARCH_REFACTOR_PLAN.md`。
...
使用 dalek CLI（noun-verb 模式）：

- 查看：`dalek ticket ls`
- 创建：`dalek ticket create --title "..." --desc "..."`
- 启动：`dalek ticket start --ticket N`
- 派发：`dalek ticket dispatch --ticket N`
- 中断：`dalek ticket interrupt --ticket N`
- 归档：`dalek ticket archive --ticket N`
- 停止：`dalek ticket stop --ticket N`
- JSON 输出：任何查询命令加 `-o json`
