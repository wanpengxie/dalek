...
- **G 是 H 的投影**：因为每一次放 actor 都带完整 text 记在账上，H 是 G 的超集，`G_t = fold(H)`，不需要 G₀ 做起点。从 H 恢复得到的是 G，不是机器——跑账本的程序（运行时 + init）不在 H 里。P 里那份 G 原样仍然要：它是 B 的产物，子代在有任何 H 之前靠它 realize；一致性要求 `fold(H) == G₀ + 后续改动`。
...
| **放 actor**（介质动作） | 在该 channel 的下一个地址登记 kind + text，在该账本记一行——**这一行带完整的 kind + text**。born 一个 channel = 它账本的第一行；init 放 c0、c1 也走这一行 |
...
- **入账规则**：一条形态改动请求的本质是**三边同时记账**——被改的 channel 记一行（它的账本因此自足：影响它的一切都在它账上），c0 记一行（它做了这一步），c0 再经门给 c1 发一行（c1 由此知道）。
- **c1：登记。** 保持注册关系不掉——actor 的注册关系、channel 的连线关系。这张关系就是**配置**。c1 = **一次折叠 + 保存**：对**自己账本上**收到的形态改动折叠一遍得到配置，落成一个文件；`decl` = 把这个文件原样抄出。c1 不读别的账本（成员只能看自己的视图，偷看是隐藏能力）：一切形态改动都经 c0，c0 每做一次就给 c1 发一条，所以 c1 知道的一切都在 c1 自己的账本上，c1 可从自己的账本重演。文件是派生物，不是私有状态：杀掉 c1 重起，重新折一遍得到同一个文件——"配置可从账本恢复"不是纪律，是定义。
...
杀掉 m，m′ 仍能造 m″                                     独立
杀掉 m′ 的进程，只留运行时 + init 与账本，重起后 c1.decl() 不变   配置可从账本恢复（机器不能只从账本恢复）
```
...
**运行时的变异**（介质层）和**构造器的变异**（组织层的 c0 / c1 源码变化）本版都不处理：M1 的种子从父机器自己的文件抄运行时与 c0、c1 源码，G 里的 c0、c1 项与种子逐字核对，不同即报错。

理论上两者都可处理，路径相同：**c2 总是可以重新编码它们**——运行时和 c0、c1 都只是源码，c2 写出新版本，pack 一个子代用新版本跑，用自举不动点验收：
...
通过前新版本只是候选。这就是编译器自举 / 内核换代的方式，也是 1.7 "对个体冻结、对谱系可变"的落实。先把不变的做通，再让它变。
