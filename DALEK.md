...
**入账的边界是 call 边界**（2026-08-31 定）：穿过它的 Ω 返回值——`Port.recv` 的 payload（消息）、每个 actor 的返回值、它发出的每次 call 及其返回——入账。oracle 是一个完整的 actor：取 context → 问端点 → 解释输出 → call / 返回，这整条是它的 run；`Port.request` 的回话是 run 里面的一个中间值，和程序的局部变量同级，不入账——它造成的每次 call 和返回都在账上。
