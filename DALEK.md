...
一个推论要记住：**组织的完整性不由 R 保证，由 D 维护**。丢了 C 的机器停不了自己（DESIGN H19），但只要 A 还在转发、c2 还在，它就能重新写一个 C 装回去——缺器官不是"机制有洞"，是"这台机器暂时不完整"，而补全是 D 的日常工作。R 若代劳就必须认识组织角色，越过内容盲。真正无法自愈的单点只有一个：**c0 的接待员**——所有形态改动都要经它，它坏了，D 的手就够不着自己的形态。
