...
3. 只编辑和 ticket start / worker runtime 相关的上下文文件，不改业务代码。
...
2. 手动执行复制任务：
- `.dalek/control/skills/start-ticket-runtime/assets/worker-agents.md.template` -> `$DALEK_WORKTREE_PATH/.dalek/agent-kernel.md`
- `.dalek/control/skills/start-ticket-runtime/assets/plan.md.template` -> `$DALEK_WORKTREE_PATH/.dalek/PLAN.md`
- `.dalek/control/skills/start-ticket-runtime/assets/state.json.template` -> `$DALEK_WORKTREE_PATH/.dalek/state.json`
