...
description: 生成或修复一次 ticket dispatch 的 worker 执行上下文（.dalek/AGENTS.md、PLAN.md、state.json）并执行三阶段校验。用于首次 dispatch、上下文漂移修复、或手动重建 worker handoff 的场景。
---
...
4. `initialize` 阶段必须通过脚本复制模板，不要手写复制逻辑。
...
2. 手动执行复制任务：
- `.dalek/control/skills/dispatch-new-ticket/assets/worker-agents.md.template` -> `$DALEK_WORKTREE_PATH/.dalek/AGENTS.md`
...
3. 编辑 `AGENTS.md`：填充身份、执行循环、`<state_update_protocol>` 与 `<current_state>`，并保持 report 约束。
4. 在 `AGENTS.md` 的 `<task_context>` 替换四个字段占位：`title`、`description`、`attachments`、`plan_ref`。
...
7. `AGENTS.md` 的 `<task_context>` 四字段占位已被业务语义内容替换（不残留 `{{...}}`）。
8.  `AGENTS.md` 中 `<state_update_protocol>` 与 `<current_state>` 块存在且内容完整。
9.  `AGENTS.md` 包含系统语义契约区块，且 `next_action` 定义固定为 `continue|done|wait_user`。
