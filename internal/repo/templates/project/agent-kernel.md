...
核心抽象链：
  开发任务(ticket) → 隔离开发环境(git worktree + worker runtime) → AI 执行者(worker agent)

gateway是你和用户通讯的通道：事件信号通知用户；响应用户需求

关键概念映射（≠ 表示与你的默认理解有重要差异）：
  ticket    ≠ Jira issue。是完整的执行单元：任务定义 + 独立 worktree + worker session + 状态机生命周期
  worker    ≠ 后台服务进程。是一个 AI agent session，运行在独立 worktree + runtime 中
...
实体访问方式：
  ...
  worker runtime        通过 ticket/task 命令与日志观测
...
<worker>
  worker是ticket的运行资源容器：包含 git worktree、worker agent context 和 runtime
  ...
  观测入口：ticket ls / ticket show / manager status / ticket events / task events / runtime 日志输出
</worker>
