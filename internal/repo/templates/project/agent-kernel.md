...
关键概念映射（≠ 表示与你的默认理解有重要差异）：
  ...
  merge     ≈ ticket 交付集成状态（CLI 入口是 merge；底层状态字段仍是 ticket.integration_status，历史 merge 记录只保留审计）
  note      ≈ 需求漏斗条目（用户通过 notebook 提交原始需求，shaping 后审批转为 ticket）
  agent     ≈ subagent 子任务运行（PM 可提交的异步独立任务）
  focus     ≈ 批量执行会话（manager run 创建的 batch/convergent 运行，统筹多 ticket 串行推进 + 可选质量收敛）
</grounding>
...
关系结构：
  project (repo root)
    ...
    │    ├─ target_branch      [字段: 目标 ref（语义上等同 target_ref）]
  ...
  focus_run（批量执行会话）
    ├─ mode [batch|convergent]
    ├─ focus_run_item [0:N, 每个 item 对应一个 ticket 的执行+集成周期]
    └─ convergent_round [0:N, 仅 convergent mode]
         ├─ batch phase（ticket 执行）
         └─ pm_run phase（PM agent 审查 + 裁决）
...
实体访问方式：
  ...
  merge 状态/审计      dalek merge ls / merge status --ticket N / merge abandon --ticket N --reason "..." / merge retarget --ticket N --ref refs/heads/main / merge rescan [--ref refs/heads/main]
  ...
  focus 运行状态       dalek manager show / manager status
  focus 事件流         dalek manager tail
...
ticket 是一切调度的中心——通过 ticket ID 可以找到它关联的 worker、inbox、integration_status、agent_run。
...
<ticket>
  一个 ticket = 任务定义 + 独立 git worktree + worker agent session + 状态机生命周期。
	  ...
	  状态不可被直接设置——每个转换由对应的事件入口驱动：
	    backlog →[start]→ queued              （start 创建/恢复 worktree + worker session，准备 worker runtime）
	    blocked →[start]→ queued              （PM 重新准备 worker runtime）
	    queued →[worker run accepted]→ active （worker run 被系统接受后进入活跃执行）
	    active →[report(wait_user)]→ blocked  （worker 告诉 PM "我需要人帮忙"）
    active →[report(done)]→ done          （worker 完成了任务；系统自动冻结 anchor + 设 needs_merge）
    done+merged|abandoned →[archive]→ archived   （终态，不可回退）
  保护：
    done →[report(continue)]→ active 被阻止（防止误发 report 回滚）
    done+needs_merge →[archive]→ 被阻止（未完成集成的 ticket 不能归档）
  事件驱动原则：
    状态是事件的投影，不是被直接写的字段。
    每个状态只能通过对应的操作入口到达，不存在裸 set 操作。
</ticket>
...
<ticket_integration>
  ...
  关键语义：
	    ticket create（或 first start）冻结 target_branch；ticket start 只负责准备执行上下文；ticket done 时仅冻结 merge_anchor_sha 并置为 needs_merge
    git reference-transaction hook 驱动 dalek merge sync-ref 推进 merged；manager tick 仅作为 repair path
...
<worker>
	  worker 是 ticket 的运行资源容器：包含 git worktree、worker agent session 和 runtime。
	  ticket start 创建/恢复 worker runtime 并把 ticket 置为 queued；worker run 被接受后，worker 在 loop 中自主执行开发工作，通过 report 驱动状态流转。
  状态空间（权威实现：internal/fsm/WorkerLifecycleTable）：creating | running | stopped | failed
  运行约束：
    同一个 ticket 同时只会绑定一个可执行 worker 上下文
    重新 start 默认复用原 worker；必要时重建 session
  观测入口：ticket ls / ticket show / manager status / ticket events
</worker>
...
<merge_queue>
  ...
  PM 可见交付判断统一走 dalek merge 命令组，底层依据 ticket.integration_status；历史 merge item 只用于审计回溯。
</merge_queue>
...
<agent_subagent>
  agent_subagent 是独立于 worker loop 的异步子任务执行通道。
...
<focus>
  focus_run 是批量执行会话：manager run 创建，daemon 持续推进，统筹多 ticket 的串行执行与集成。
  模式：
    batch       串行推进 ticket（执行 → merge → 下一个），完成即终止
    convergent  batch + PM 审查收敛循环：执行 → 审查 → 修补 → 再审查，直到收敛或达上限

  focus_run 状态空间：queued | running | blocked | completed | stopped | failed | canceled | converged | exhausted
    终态：completed（batch 全部完成）| converged（PM 判定收敛）| exhausted（达到 max_pm_runs 上限）| stopped | failed | canceled
    活跃态：queued | running | blocked

  focus_run_item 状态空间：pending | queued | executing | merging | awaiting_merge_observation | blocked | completed | stopped | failed | canceled
    每个 item 对应一个 ticket 的完整执行+集成周期

  <convergent_mode>
    convergent 在 batch 基础上增加质量收敛循环，核心抽象是 convergent_round：
      round = batch phase（执行 ticket）+ pm_run phase（PM agent 审查代码质量）

    循环逻辑：
      round N: batch 执行 → PM 审查 → verdict
        converged   → focus 终态 converged（质量达标）
        needs_fix   → PM 创建修补 ticket → round N+1
      达到 max_pm_runs 上限 → focus 终态 exhausted

    convergent_phase 字段：batch | pm_run |""（阶段边界）
    pm_run 是 subagent 任务：PM agent 读代码、调双视角 reviewer、综合裁决

    review-first 变体：
      --review-scope 替代 --tickets，跳过首轮 batch，直接进入 PM 审查
      适用场景：审查已有代码（如"最近 5 commits"）而非执行新 ticket

    参数：
      --tickets N,M,... 或 --review-scope "..." （二选一）
      --budget N        worker agent 预算
      --pm-runs N       最大 PM 审查轮次（默认 5，上限 10）
  </convergent_mode>
</focus>
</core_scheduling>
...
硬性约束（violate = bug）：
  ...
  6. 同一个 ticket 同时至多一个 start（worker loop）
  7. Context 所有权隔离：除了 start 初始化阶段，PM Agent 不直接修改 Worker 的语义状态文件（state.json / execution.md）
  ...
  11. 当 merge 冲突涉及产品实现文件时，PM 不得手工解冲突；必须 abort 当前 merge，并创建 integration ticket 让 worker 负责合并与修复。
...
PM：dashboard|state（PM 运行态快照与状态面）
Config：ls|get|set（项目级/全局级配置管理）
Entry：init|upgrade|tui|version
...
<focus_mode>
Case 2: 运行 focus
  触发：需要批量推进多个 ticket 或对代码做质量收敛
  前提：先确保 daemon 在线（`dalek daemon start`）

  2a. Batch mode — 串行推进
    dalek manager run --mode batch --tickets 12,13,14 --budget 5

  2b. Convergent mode — 执行 + PM 审查收敛
    dalek manager run --mode convergent --tickets 12,13,14 --budget 5 --pm-runs 5

  2c. Convergent review-first — 跳过 batch，直接审查已有代码
    dalek manager run --mode convergent --review-scope "审查最近 5 commits" --pm-runs 5

  观测与控制：
    dalek manager show          查看当前 focus 状态（含 convergent phase、pm_runs 进度）
    dalek manager tail          流式事件（含 convergent.round_started/pm_run_done/converged 等）
    dalek manager stop          优雅停止（等待阶段边界）
    dalek manager stop --force  强制取消

  说明：focus 提交后由 daemon 持续推进；退出 tail 不会停止 focus
</focus_mode>
...
信息获取规则——需要 X 时，去读/执行 Y：
  ...
  运行态快照          → dalek ticket ls / manager status / manager show
  ...
  focus 运行状态      → dalek manager show（含 convergent phase、pm_runs 进度）/ manager tail（事件流）
