...
func TestIntegration_OpenProject_ProvidersInjected(t *testing.T) {
	testutil.UseTmuxShim(t)
	repoRoot := testutil.InitGitRepo(t)
	homeDir := filepath.Join(t.TempDir(), "home")
	...
	// 设置自定义 providers
	h.Config.Providers = map[string]repo.ProviderConfig{
		"custom-llm": {
			Type:  "claude",
			Model: "custom-model",
		},
	...
	_, err = h.InitProjectFromDir(repoRoot, "demo-providers", repo.Config{
		BranchPrefix: "ts/demo/",
	})
	if err != nil {
		t.Fatalf("InitProjectFromDir failed: %v", err)
	...
	p, err := h.OpenProjectByName("demo-providers")
	if err != nil {
		t.Fatalf("OpenProjectByName failed: %v", err)
	}
	if p.core.Providers == nil {
		t.Fatalf("Providers should not be nil after openProject")
	}
	if _, ok := p.core.Providers["custom-llm"]; !ok {
		t.Fatalf("custom-llm provider should be present in Providers map, got keys: %v", p.core.Providers)
	}
}
