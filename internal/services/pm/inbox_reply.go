...
func (s *Service) ReplyInboxItem(ctx context.Context, id uint, rawAction, reply string) (InboxReplyResult, error) {
	...
	if item.WaitRoundCount >= maxWaitUserRounds {
		return InboxReplyResult{}, fmt.Errorf("wait_user 链已达到 %d 次上限，当前 inbox 不再自动恢复", maxWaitUserRounds)
	}
	...
	if focusRun != nil && focusItem != nil {
		...
		updatedInbox, err := s.GetInboxItem(ctx, item.ID)
		if err != nil {
			return InboxReplyResult{}, err
		}
		if updatedInbox == nil {
			return InboxReplyResult{}, fmt.Errorf("回复 inbox#%d 后重新读取失败", item.ID)
		}
		if err := s.focusResumeBlockedItemFromInboxReply(ctx, *focusRun, *focusItem, *updatedInbox); err != nil {
			return InboxReplyResult{}, err
		}
	...
	if _, err := s.StartTicket(ctx, ticket.ID); err != nil {
		_ = s.reopenInboxReplyIntent(context.WithoutCancel(ctx), item.ID)
		return InboxReplyResult{}, err
	}
	autoStart := false
