...
func (s *Service) guardDirectWorkerTerminalReport(ctx context.Context, r contracts.WorkerReport) (contracts.WorkerReport, error) {
	if ctx == nil {
		ctx = context.Background()
	}
	r.Normalize()
	...
	if r.TaskRunID == 0 {
		return contracts.WorkerReport{}, fmt.Errorf("worker report terminal closure 缺少 task_run_id")
	}
	w, err := s.worker.WorkerByID(ctx, r.WorkerID)
	if err != nil {
		return contracts.WorkerReport{}, fmt.Errorf("读取 worker 失败: %w", err)
	...
	issues, err := s.validateWorkerLoopTaskRunBinding(ctx, ticketID, w.ID, r.TaskRunID, r.TaskRunID)
	if err != nil {
		return contracts.WorkerReport{}, err
	}
	switch next {
	case string(contracts.NextDone):
		if !meaningfulWorkerSummary(r.Summary) {
			issues = append(issues, "done closure 缺少非空 summary。")
		}
		r.Blockers = cleanStringSlice(r.Blockers)
		if len(r.Blockers) > 0 {
			issues = append(issues, "done closure 要求 blockers 为空。")
		}
		if r.Dirty {
			issues = append(issues, "worker report 标记 worktree dirty，不能直接收口为 done。")
		}
		git := s.inspectWorkerLoopGitSnapshot(ctx, strings.TrimSpace(w.WorktreePath))
		if git.DirtyKnown && git.Dirty {
			issues = append(issues, "当前 worktree 仍然 dirty，不能收口为 done。")
		}
		if git.HeadKnown && looksLikeGitCommit(r.HeadSHA) && strings.TrimSpace(r.HeadSHA) != git.HeadSHA {
			issues = append(issues, fmt.Sprintf("worker report head_sha=%s 与实际 HEAD=%s 不一致。", strings.TrimSpace(r.HeadSHA), git.HeadSHA))
		}
		if !looksLikeGitCommit(r.HeadSHA) && git.HeadKnown && (!git.DirtyKnown || !git.Dirty) {
			r.HeadSHA = git.HeadSHA
		}
		if !looksLikeGitCommit(r.HeadSHA) {
			issues = append(issues, "done closure 缺少可冻结的 clean merge anchor。")
		} else {
			r.Dirty = false
		}
	case string(contracts.NextWaitUser):
		if !meaningfulWorkerSummary(r.Summary) {
			issues = append(issues, "wait_user closure 缺少能解释阻塞原因的 summary。")
		}
		r.Blockers = cleanStringSlice(r.Blockers)
		if len(r.Blockers) == 0 {
			issues = append(issues, "wait_user closure 缺少 blockers，无法说明为什么需要人工介入。")
		}
	}
	if len(issues) > 0 {
		return contracts.WorkerReport{}, fmt.Errorf("worker report terminal closure 校验失败: %s", strings.Join(issues, "；"))
	}
	return r, nil
}
