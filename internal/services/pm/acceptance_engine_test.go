...
func TestApproveMergePMOpExecutor_OptionalAcceptanceGate(t *testing.T) {
	svc, p, _ := newServiceForTest(t)
	ctx := context.Background()

	writeTestPlanGraph(t, p.RepoRoot, contracts.FeatureGraph{
		Schema:    contracts.PMFeatureGraphSchemaV1,
		FeatureID: "feature-merge-gate",
		Goal:      "merge gate",
		Nodes: []contracts.FeatureNode{
			{ID: "ticket-main", Type: contracts.FeatureNodeTicket, Status: contracts.FeatureNodeDone},
			{ID: "acceptance-01", Type: contracts.FeatureNodeAcceptance, Status: contracts.FeatureNodePending, DependsOn: []string{"ticket-main"}},
	...
	ticket := createTicket(t, p.DB, "merge gate ticket")
	merge := contracts.MergeItem{
		Status:   contracts.MergeProposed,
		TicketID: ticket.ID,
		Branch:   "feature/test",
	}
	if err := p.DB.WithContext(ctx).Create(&merge).Error; err != nil {
		t.Fatalf("create merge item failed: %v", err)
	}

	_, err := approveMergePMOpExecutor{s: svc}.Execute(ctx, contracts.PMOp{
		Kind: contracts.PMOpApproveMerge,
		Arguments: contracts.JSONMap{
			"merge_item_id":           merge.ID,
			"enforce_acceptance_gate": true,
		},
	})
	if err == nil {
		t.Fatalf("expected approve_merge gate failure")
	}

	graph := readTestPlanGraph(t, filepath.Join(p.RepoRoot, ".dalek", "pm", "plan.json"))
	for i := range graph.Nodes {
		if graph.Nodes[i].ID == "acceptance-01" {
			graph.Nodes[i].Status = contracts.FeatureNodeDone
		}
	...
	if err != nil {
		t.Fatalf("approve_merge should pass after acceptance done: %v", err)
	}

	var after contracts.MergeItem
	if err := p.DB.WithContext(ctx).First(&after, merge.ID).Error; err != nil {
		t.Fatalf("reload merge item failed: %v", err)
	}
	if after.Status != contracts.MergeApproved {
		t.Fatalf("merge status mismatch: got=%s want=%s", after.Status, contracts.MergeApproved)
	}
}
