...
func New(p *core.Project) *Service {
	...
	if p != nil {
		logger = core.EnsureLogger(p.Logger).With("service", "notebook")
	}
	return &Service{
		core:   p,
		logger: logger,
...
func (s *Service) projectKey() string {
	...
	return strings.TrimSpace(s.core.Key)
}
...
func (s *Service) AddNote(ctx context.Context, rawText string) (NoteAddResult, error) {
...
func (s *Service) NotebookShapingSkillPath() string {
	if s == nil || s.core == nil {
		return ""
	}
	base := strings.TrimSpace(s.core.Layout.ControlSkillsDir)
	if base == "" {
		return ""
	}
...
func (s *Service) ProcessOnePendingNote(ctx context.Context) (bool, error) {
	if s == nil || s.core == nil || s.core.DB == nil {
	...
	note, err := s.claimOneOpenNote(ctx)
	if err != nil {
		return false, err
	}
	if note == nil {
	...
	if err := s.shapeClaimedNote(ctx, *note); err != nil {
		return true, err
	}
	return true, nil
}

func (s *Service) RecoverStuckShapingNotes(ctx context.Context, stale time.Duration) (int, error) {
	if s == nil || s.core == nil || s.core.DB == nil {
	...
	if err := s.core.DB.WithContext(ctx).
	...
	for _, note := range notes {
		...
		_ = s.upsertNoteInbox(ctx, fmt.Sprintf("note_recovery_%d", note.ID), fmt.Sprintf("note %d shaping 中断，已回滚", note.ID), msg, 0)
	}
	return len(notes), nil
}

func (s *Service) claimOneOpenNote(ctx context.Context) (*store.NoteItem, error) {
...
func (s *Service) shapeClaimedNote(ctx context.Context, note store.NoteItem) error {
	if s == nil || s.core == nil || s.core.DB == nil {
		return fmt.Errorf("project db 为空")
	}
	skillPath := s.NotebookShapingSkillPath()
	skillText, err := os.ReadFile(skillPath)
	if err != nil {
		msg := fmt.Sprintf("notebook shaping skill 缺失或不可读: %s", skillPath)
		_ = s.core.DB.WithContext(ctx).
			Model(&store.NoteItem{}).
			Where("id = ?", note.ID).
			Updates(map[string]any{
				"status":     store.NoteOpen,
	...
	return s.core.DB.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
...
func (s *Service) upsertNoteInbox(ctx context.Context, key, title, body string, noteID uint) error {
	if s == nil || s.core == nil || s.core.DB == nil {
		return fmt.Errorf("project db 为空")
	}
	...
	return s.core.DB.WithContext(ctx).Create(&item).Error
}

func (s *Service) ListNotes(ctx context.Context, opt ListNoteOptions) ([]NoteView, error) {
	if s == nil || s.core == nil || s.core.DB == nil {
	...
	return s.buildNoteViews(ctx, notes)
}

func (s *Service) GetNote(ctx context.Context, id uint) (*NoteView, error) {
	if s == nil || s.core == nil || s.core.DB == nil {
		return nil, fmt.Errorf("project db 为空")
	}
	if id == 0 {
	...
	var note store.NoteItem
	if err := s.core.DB.WithContext(ctx).First(&note, id).Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, nil
		}
		return nil, err
	}
	views, err := s.buildNoteViews(ctx, []store.NoteItem{note})
	if err != nil {
		return nil, err
	}
	if len(views) == 0 {
...
func (s *Service) RejectNote(ctx context.Context, id uint, reason string) error {
	if s == nil || s.core == nil || s.core.DB == nil {
		return fmt.Errorf("project db 为空")
	}
	if id == 0 {
...
func (s *Service) DiscardNote(ctx context.Context, id uint) error {
	if s == nil || s.core == nil || s.core.DB == nil {
		return fmt.Errorf("project db 为空")
	}
	if id == 0 {
...
func (s *Service) buildNoteViews(ctx context.Context, notes []store.NoteItem) ([]NoteView, error) {
	if s == nil || s.core == nil || s.core.DB == nil {
		return nil, fmt.Errorf("project db 为空")
	}
	if len(notes) == 0 {
	...
	if len(ids) > 0 {
		var shaped []store.ShapedItem
		if err := s.core.DB.WithContext(ctx).Where("id IN ?", ids).Find(&shaped).Error; err != nil {
			return nil, err
		}
