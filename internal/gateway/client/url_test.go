...
func TestResolveDaemonWSURL_HomeConfigFallback(t *testing.T) {
	...
	got := ResolveDaemonWSURL("", home)
	if got != "ws://127.0.0.1:29091/ws" {
		t.Fatalf("unexpected ws url from home config: %q", got)
	}
}

func TestResolveDaemonWSURL_EnvOverridesHomeConfig(t *testing.T) {
...
func TestResolveDaemonWSURL_CLIOverridesAll(t *testing.T) {
	t.Setenv("DALEK_GATEWAY_WS_URL", "ws://127.0.0.1:39090/ws")
	home := t.TempDir()
	if err := app.WriteHomeConfigAtomic(filepath.Join(home, "config.json"), app.HomeConfig{
		Daemon: app.HomeDaemonConfig{
	...
	got := ResolveDaemonWSURL("ws://127.0.0.1:49090/ws", home)
	if got != "ws://127.0.0.1:49090/ws" {
		t.Fatalf("cli ws url should override env/home config: %q", got)
	}
}

func TestResolveDaemonWSURL_HomeListenHTTPS(t *testing.T) {
	t.Setenv("DALEK_GATEWAY_WS_URL", "")
	home := t.TempDir()
	if err := app.WriteHomeConfigAtomic(filepath.Join(home, "config.json"), app.HomeConfig{
		Daemon: app.HomeDaemonConfig{
...
func TestBuildChatWSURL_PreservesBaseQuery(t *testing.T) {
	got, err := BuildChatWSURL("ws://127.0.0.1:18081/ws?trace=1", "demo", "conv1", "sender1")
	if err != nil {
		t.Fatalf("BuildChatWSURL failed: %v", err)
	}
	u, err := url.Parse(got)
	if err != nil {
