...
ups = {P: sum(1 for r in rows(Runtime(P).load(), "c0", "msg") if r["from"] == "_root" and r["body"] == "up") for P in (P0, P1, P2)}
pids = [Exec.spawn(["init.py", str(P), "--serve"], cwd=P, log=P/"log") for P in (P0, P1, P2)]
try:
    for P in (P0, P1, P2):
        wait_child(P, lambda c, P=P: sum(1 for r in rows(c, "c0", "msg") if r["from"] == "_root" and r["body"] == "up") == ups[P] + 1, timeout=60)
    log("all three woke from their ledgers (root up rows appended)")
