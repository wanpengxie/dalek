...
用 `git worktree + sqlite` 管理“ticket -> worktree -> agent 运行”的最小闭环。
...
1. `Ticket.Status`（管理/流程状态）
   ...
   - `running` 进行中
   - `done` 完成
2. `Worker.Status`（worker 生命周期）
   - `creating / running / stopped / failed`
3. `Worker.RuntimeState + NeedsUser + Summary`（业务运行态，由 watcher/agent 推断并写回 DB；report 为主，runtime 日志观测为辅）
...
- `n` 新建 ticket
- `s` 启动（创建 worktree + worker runtime）
- `p` 派发任务（`a` 自动 / `m` 手动；会写入 worktree 的 `.dalek/task.md`，并在 auto 模式启动 agent）
- `i` 软中断（向目标 worker 进程发送 `SIGINT`）
- `a` attach 进入运行输出（detach 后回到 TUI）
- `k` 停止（停止对应 worker 进程）
...
目标：像人盯运行日志一样“读上下文 + 看变化”，把运行态写回 DB，并把关键观测记录成事件用于回放。

v0 把 watcher 拆成两层，避免“所有 session 每分钟都跑一次小模型”导致成本爆炸：

- cheap sampler（不调用模型）：按 `watcher_sample_interval_ms` 轮询所有 `Worker.Status=running` 的 worker
- 语义判断（调用 `watcher_command`）：只在“状态可能变化/需要解释”时触发，并按 `watcher_interval_ms` 节流（每个 worker 最多一次）

cheap sampler 每轮对每个 running worker 会做：

1. 判断 worker runtime 是否在线
2. 把输出流写到 `.dalek/workers/w<workerID>/stream.log`
3. 抓取最近输出末尾 N 行文本
...
查看 ticket/worker 运行状态：

```bash
dalek ticket show --ticket <id>
```

查看任务事件：

```bash
dalek task events --id <taskRunID>
```
