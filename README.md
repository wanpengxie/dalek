...
| **构造器 = 系统原语** born / add / copy / peer / start | A 建膜、构造、绑定；B 复制；C 接线、启动 | 运行时提供的 syscall，只有 c0 能发；`D` 只是 c0 里把请求拆成原语的翻译员 |
| 管理器 | 持"配置 ↔ 实体"表，向 c0 发构造请求 | 用原语按配置造出来的功能单元（v6 未实现） |
| F | 普通成员、普通 channel | 被造的功能单元 |

**请求语言**（Author 写给 `D`，`D` 拆成原语）：
```
