...
**human 的定义**：外生成员——Decl ∉ H 的成员（不是 door 造的）。与 stdin 后面是谁无关；kind=human 的含义只是"不是这台机器造的"。G 从外生成员进入，封闭到此为止。

## 2. K — 五条规则

候选的唯一法律：**无带外效应**——每个效应都以 K 盖章的消息进入 H。下面五条规则、§3 的 door、§5 的 T3–T5 都应是它的推论；失败驱动实验要验证的正是"去掉它是否裂、只加它是否够"。
