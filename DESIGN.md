...
**H19 · C · 能停自己是一个器官，不是天赋（2026-09-01 晚，实测）。** 合法关机是一条链：A 转发 `stop` → 存在 tag=C 的活成员 → 它持 `bind=stop` → 它的 text 真的调 `call("stop")`。**四环都是可变异的 text，断任何一环，机器就再也无法经协议停止，只能被杀。** 实测：把 C 去掉的机器收到 `stop`，`_resolve("C")` 落空、消息按"不存在的地址"丢弃，`_stopping` 仍是 False、根边界一行不写，机器照常活着。这不是缺陷，是"能力即器官"的诚实后果，但要记清两条推论：(1) **"机器总能停止自己"不是不变量**，是某个 G 的性质——c2 改 c0 的器官时可能把它悄悄改没（Trusting Trust 味道：改动的正是提供安全相关能力的那个器官，而且没人会发现，直到有人试图停机）。(2) **不该由 R 来保护它**：R 拒绝自退役、拒绝退接待员是不认名字的结构规则，而"别退最后一个持 bind=stop 的成员"必须认识组织角色，越过内容盲。要保护就写成 A 的策略（转发 retire 前自己查），那是 policy 层的事。
